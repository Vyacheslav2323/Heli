...
if __name__ == '__main__':
    ...
    # Prefer AirPods Hands-Free mic: laptop Microphone Array is nearly silent
    # (peak ~100) while Hands-Free captures normally. Prefer 16 kHz endpoint.
    ...
    def _find_input_device():
        ...
        try:
            ...
            if chosen is not None:
                ...
                console.print(
                    f"[green]Using input device {chosen}: {info['name']} "
                    f"({int(info['defaultSampleRate'])} Hz)[/green]"
                )
                return chosen
            default = pa.get_default_input_device_info()
            console.print(
                f"[yellow]No AirPods Hands-Free mic found; using default "
                f"{default['index']}: {default['name']}[/yellow]"
            )
    ...
    recorder_config = {
        ...
        'language': 'en',
        # Higher silero_sensitivity = easier speech detection (threshold = 1 - value)
